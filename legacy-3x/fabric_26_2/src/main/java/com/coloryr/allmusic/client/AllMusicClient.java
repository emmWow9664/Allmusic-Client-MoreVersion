...
public class AllMusicClient implements ClientModInitializer, AllMusicBridge {
    ...
    public Object genTexture(int size) {
        ...
        var tex = device.createTexture("allmusic:gui_textured", 5, GpuFormat.RGBA8_UNORM, size, size, 1, 1);
//        tex.setTextureFilter(FilterMode.NEAREST, false);
    ...
    public void drawText(String item, int x, int y, int color, boolean shadow) {
        ...
        // 新的 HUD 绘制接口把颜色按 ARGB 处理，配置文件里默认的 0xFFFFFF 没有 alpha 位，
        // 直接传进去等于全透明（文字/歌词/歌曲信息全都看不见），这里按老版 drawString 的规则补上不透明
