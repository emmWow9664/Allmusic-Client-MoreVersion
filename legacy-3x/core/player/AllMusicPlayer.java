    ...
    private void run() {
        ...
        while (true) {
            try {
                ...
                if (!volumeWarned && AllMusicCore.bridge.getVolume() <= 0) {
                    volumeWarned = true;
                    // 音乐音量取的是「唱片机/音符盒」滑块，玩家把它拉到 0 时表现为「封面歌词都正常但完全没声音」
                    AllMusicCore.bridge.sendMessage("当前「唱片机/音符盒」音量为 0，音乐不会出声，请在 设置→声音 里调高");
                }
